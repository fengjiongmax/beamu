...
Future<LOGIN_STAT> loginAction(
    String username, String password, String giteaHost) async {
  while (giteaHost.substring(giteaHost.length - 1, giteaHost.length) == '/') {
    giteaHost =
        giteaHost.substring(0, giteaHost.length - 1); //remove the last '/'
  }
  ...
  doLogin() async {
    Response rtVal;
    final response = await httpGet(loginUrl, header)
  ...
  createToken() async {
    ...
    Response rtVal;
    final response = await httpPost(loginUrl, postBody, header)
        .timeout(Duration(seconds: 30), onTimeout: () {
      return null;
    });
    rtVal = response;
  ...
  await doLogin().then((value) async {
    if (value != null) {
      if (value.statusCode == 200) {
        ...
        List<TokenModel> tokens = parsed
            .map<TokenModel>((json) => TokenModel.fromJson(json))
            .toList();
...
Future<UserModel> getUserInfo() async {
  var user = await getUser(config.userName.trim());
  return user;
}

Future<bool> loginCHK() async {
  SharedPreferences prefs = await SharedPreferences.getInstance();
  return prefs.getBool(DONE_LOGIN) &&
      await config.setLoginToken(); // find token locally
}
