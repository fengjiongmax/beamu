...
Future<List<LabelModel>> getRepoLabels(
  ...
  List<LabelModel> labels =
      _parsed.map<LabelModel>((json) => LabelModel.fromJson(json)).toList();
  return labels;
}

Future<List<LabelModel>> createIssueLabels(String ownerName, String repoName,
    int issueIndex, List<int> labelIndices) async {
  final url = config.giteaHost +
      '/api/v1/repos/' +
      ownerName +
      '/' +
...
Future<List<LabelModel>> updateIssueLabels(String ownerName, String repoName,
    int issueIndex, List<int> labelIndices) async {
  final url = config.giteaHost +
      '/api/v1/repos/' +
      ownerName +
...
Future<bool> removeIssueLabels(
    String ownerName, String repoName, int issueIndex) async {
  final url = config.giteaHost +
      '/api/v1/repos/' +
      ownerName +
      '/' +
