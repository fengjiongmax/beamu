...
class RepositoryState extends State<Repository>
          ...
          builder: (context) => TabBarView(
            ...
            children: <Widget>[
              Container(
                ...
                child: _readmeLoading
                    ? LoadingContent()
                    : _readme == null
                        ? CenterText(
                            centerText: 'README.md not found',
                          )
                        : MarkdownRender(
                            data: _readme,
                          ),
  ...
  void _tabChange() {
    setState(() {
      _showFAB = tabContents[_tabController.index].showFab &&
          tabContents[_tabController.index].title == 'ISSUES';
              ...
              children: <Widget>[
                ListTile(
                  ...
                  onTap: () async {
                    ...
                    var _assigneeCandidate = _collaborators.sublist(
                        0); //this copy from _collaborators to _assigneeCandidate
                    if (widget.isOwnerOrg)
                      _assigneeCandidate.addAll(
                          _orgMembers); // add member if owner of this repo is org
                    else if (!_collaborators.contains(widget.owner))
                      _assigneeCandidate.add(
                          _repo.owner); //add owner if does not contains owner
                            ...
                            children: issue.labels.map((label) {
                              ...
                              return Container(
                                ...
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    color:
                                        y < 128 ? Colors.white : Colors.black,
                                  ),
                                ),
                              );
                            }).toList()),
  ...
  Widget _buildMilestones() {
    ...
    Widget _buildSingleMilestone(MilestoneModel milestone) {
      ...
      return Card(
          child: ListTile(
        leading: milestone.state == 'closed'
            ? Icon(
                Icons.cancel,
                color: Colors.red,
              )
        ...
        subtitle: Text(formatter.format(
            milestone.state == 'open' ? milestone.dueOn : milestone.closedAt)),
      ));
    }
  ...
  void _getREADME() {
    if (_repo != null) {
      getDefaultREADME(widget.repoName, widget.repoName, _repo.defaultBranch)
          .then((v) {
        if (this.mounted) {
          setState(() {
            _readme = v;
  ...
  void _getOwnerMembers() {
    if (widget.isOwnerOrg) {
      getOrgMembers(widget.owner).then((v) {
        if (this.mounted) {
          setState(() {
            _orgMembers.clear();
