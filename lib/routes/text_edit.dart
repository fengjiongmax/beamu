...
enum FINISH_METHOD { POST, PATCH }

enum BODY { ISSUE, COMMENT }
...
class EditorState extends State<Editor> {
  ...
  void initState() {
    ...
    if (widget.body == BODY.ISSUE) {
      _controller = new TextEditingController(
          text: widget.issue == null ? '' : widget.issue.body);
    } else if (widget.body == BODY.COMMENT) {
      _controller = new TextEditingController(
          text: widget.comment == null ? '' : widget.comment.body);
    }
    setState(() {
  ...
  Future<bool> _saveValue() async {
    ...
    switch (widget.body) {
      case BODY.COMMENT:
        if (widget.comment == null && widget.method == FINISH_METHOD.POST) {
          //update comment is not available for now
          var response =
              await httpPost(widget.url, '{"body":"' + _bodyString + '"}');
